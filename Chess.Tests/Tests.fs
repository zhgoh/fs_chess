...
let ``Check pawn move`` () =
  // White pawns 
  let res = generatePawnMove (getSquare "a2") PawnDirection.Up
  Assert.Equal (2, res.Length)
  Assert.Contains((getSquare "a3"), res)
  Assert.Contains((getSquare "a4"), res)

  // Black pawns 
  let res = generatePawnMove (getSquare "a7") PawnDirection.Down
  Assert.Equal (2, res.Length)
  Assert.Contains((getSquare "a6"), res)
  Assert.Contains((getSquare "a5"), res)

  let res = generatePawnMove (getSquare "a7") PawnDirection.Up
  Assert.Equal (1, res.Length)

  // Invalid
  let res = generatePawnMove (getSquare "a8") PawnDirection.Up
  Assert.Equal (0, res.Length)

  let res = generatePawnMove (getSquare "a1") PawnDirection.Down
  Assert.Equal (0, res.Length)
